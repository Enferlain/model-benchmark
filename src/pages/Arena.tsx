  ...
  return (
    <div className="max-w-[1800px] mx-auto px-6 py-6 h-[calc(100vh-64px)] flex flex-col">
       ...
       <div className="flex-1 flex flex-col min-h-0">
          {activeTab === 'battle' ? (
              <div key={currentRound} className="flex-1 animate-in fade-in slide-in-from-bottom-2 duration-500">
                 <ArenaBattle 
                    prompt={currentPrompt}
                    imageA={mockImages[(currentRound * 2) % mockImages.length]}
                    imageB={mockImages[(currentRound * 2 + 1) % mockImages.length]}
                    refImage={currentRound % 2 === 0 ? undefined : "https://placehold.co/200x200/444/FFF.png?text=Ref"}
                    onVote={handleVote}
                 />
              </div>
          ) : (
