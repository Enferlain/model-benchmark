...
import { Expand, Shrink, X, Check, Search, AlertCircle, ThumbsUp, ThumbsDown, Minus } from 'lucide-react';
...
export function ArenaBattle({ prompt, imageA, imageB, refImage, onVote }: ArenaBattleProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-7xl mx-auto gap-6 transition-all duration-500">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 text-center relative z-10">
        <h3 className="text-sm uppercase tracking-wider font-bold text-slate-400 mb-1">Prompt</h3>
      ...
      <div className="flex-1 min-h-[400px] flex items-stretch justify-center gap-4 relative">
        ...
        {/* Model A */}
        <div className={`flex-1 relative group rounded-xl overflow-hidden bg-black/5 shadow-inner border border-slate-200 dark:border-slate-700 transition-all duration-500 ${isRefExpanded ? 'flex-[1]' : 'flex-[10]'}`}>
          <img 
            src={imageA} 
            alt="Model A Output" 
        ...
        >
             ...
             <button
                ...
                title={isRefExpanded ? "Collapse Reference" : "Show Reference Image"}
             >
                {isRefExpanded ? (
                    <Shrink size={18} />
                ) : (
                    <>
                         ...
                         <span className="text-[10px] font-bold text-slate-400 group-hover:text-indigo-500 transition-colors uppercase tracking-tight">Ref</span>
                    </>
                )}
             </button>
                     ...
                     <div className="w-full flex-1 rounded-lg overflow-hidden bg-slate-100 dark:bg-black/20 relative">
                        {refImage ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-4 text-center">
                                ...
                                <span className="text-sm">No reference image available</span>
                            </div>
                        )}
                     </div>
                     <p className="text-center text-xs text-slate-400 mt-2 font-medium uppercase tracking-wider">Ground Truth</p>
                 </div>
             )}
        </div>
      ...
      <div className="flex justify-center items-center gap-4 py-4">
        ...
        {showLightbox && (
            ...
            >
                <img src={showLightbox} className="max-w-full max-h-full rounded-md shadow-2xl" />
...
function VoteButton({ onClick, color, icon, label }: { onClick: () => void, color: 'indigo' | 'slate' | 'red', icon: React.ReactNode, label: string }) {
    const colors = {
        indigo: "bg-indigo-100 text-indigo-700 hover:bg-indigo-600 hover:text-white border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-700 dark:hover:bg-indigo-600 dark:hover:text-white",
        slate: "bg-slate-100 text-slate-700 hover:bg-slate-600 hover:text-white border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 dark:hover:bg-slate-600 dark:hover:text-white",
