...
const CustomTooltip = ({ active, payload, isDarkMode }: any) => {
	if (active && payload && payload.length) {
		...
		return (
			<div
				className={`p-4 border shadow-2xl rounded-2xl text-sm z-50 backdrop-blur-xl ${isDarkMode ? "bg-slate-800/80 border-white/10 text-slate-200" : "bg-white/80 border-white/50 text-slate-800"}`}
			>
...
const calculateDistance = (
	...
	const dx =
		(p1[xKey as keyof ModelData] as number) -
		(p2[xKey as keyof ModelData] as number);
	const dy =
		(p1[yKey as keyof ModelData] as number) -
		(p2[yKey as keyof ModelData] as number);
...
const RenderNode = (props: any) => {
	...
	const isDimmed =
		...
		(hoveredId !== null && !isHovered && !isSelected);
...
export const ScatterPlot: React.FC<ScatterPlotProps> = ({
	...
	const _clickProcessedRef = useRef(false);
			...
			<div className="flex-1 min-h-0 w-full select-none">
				<ResponsiveContainer width="100%" height="100%">
					...
					>
						<CartesianGrid
							stroke={gridColor}
							strokeOpacity={isDarkMode ? 0.2 : 0.4}
							strokeDasharray="3 3"
						/>
						<XAxis
							type="number"
