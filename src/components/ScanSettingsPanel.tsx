...
export const ScanSettingsPanel: React.FC<ScanSettingsPanelProps> = ({
	...
	const updateOption = <K extends keyof ScanOptionsType>(
		key: K,
		value: ScanOptionsType[K],
	) => {
				...
				<div className="space-y-4">
					...
					<div className="grid grid-cols-2 gap-3">
						<div>
							...
							<input
								...
								value={options.steps}
								onChange={(e) =>
									updateOption("steps", parseInt(e.target.value, 10) || 20)
								}
								min={1}
								max={150}
								disabled={status.is_running}
					...
					<div className="grid grid-cols-2 gap-3">
						<div>
							...
							<input
								...
								value={options.seed}
								onChange={(e) =>
									updateOption("seed", parseInt(e.target.value, 10) || 0)
								}
								disabled={status.is_running}
								className="w-full px-3 py-2 bg-white/50 dark:bg-black/20 border border-slate-200/60 dark:border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 text-slate-800 dark:text-slate-200 disabled:opacity-50"
							/>
					...
					<div className="grid grid-cols-2 gap-3">
						<div>
							...
							<input
								...
								value={options.width}
								onChange={(e) =>
									updateOption("width", parseInt(e.target.value, 10) || 1024)
								}
								min={512}
								max={2048}
								step={64}
						...
						<div>
							...
							<input
								...
								onChange={(e) =>
									updateOption("height", parseInt(e.target.value, 10) || 1536)
								}
								min={512}
								max={2048}
								step={64}
					...
					<div>
						...
						<p className="text-[10px] text-slate-400 mt-1 ml-1">
							Set &gt; 1 to measure intra-prompt diversity
						</p>
					</div>
					...
					<div className="bg-slate-100/50 dark:bg-slate-900/30 rounded-xl p-3 text-xs text-slate-600 dark:text-slate-400">
						<p>
							<strong>Total images per model:</strong> {options.num_prompts} ×{" "}
							{options.images_per_prompt} ={" "}
							{options.num_prompts * options.images_per_prompt}
						</p>
						<p className="mt-1">
						...
						<div className="bg-blue-50/50 dark:bg-blue-900/20 rounded-xl p-3 text-xs text-blue-700 dark:text-blue-300">
							...
							{status.progress.total > 0 && (
								<>
									...
									<p className="mt-1">
										{status.progress.current} / {status.progress.total} images
									</p>
								</>
							)}
						</div>
