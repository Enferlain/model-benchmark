...
interface Props {
  prompts: string[];
  modelNames: string[];
  getImagesForSelection: (prompt: string, seed: string) => (ModelOutput | undefined)[];
  seed: string;
}
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      <div className="flex-1 overflow-auto">
        <table className={`w-full border-collapse ${fitToScreen ? 'table-fixed' : 'table-fixed'}`}>
            ...
            <tr>
              ...
              >
                <div className="flex flex-col gap-2">
                   <div className="flex items-center justify-between">
                       <span className="text-xs font-bold uppercase tracking-wider text-zinc-500">Prompt</span>
                       <button 
              ...
              {modelNames.map((name, i) => (
                <th 
                    ...
                    style={fitToScreen ? { width: 'auto' } : { width: zoomLevel, minWidth: zoomLevel }}
                >
                  <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 block truncate" title={name}>
                      {name}
                  </span>
                </th>
              ))}
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {prompts.map((prompt, pIdx) => {
              ...
              return (
                <tr key={pIdx} className="group hover:bg-zinc-100 dark:hover:bg-zinc-900/50 transition-colors">
                  {/* Sticky Prompt Cell 
                  ...
                  */}
                  ...
                  >
                    <div className="h-full overflow-y-auto max-h-full pr-1">
                        <p className="text-xs text-zinc-600 dark:text-zinc-400 font-mono" title={prompt}>
                        {prompt}
                        </p>
                    </div>
                  </td>
