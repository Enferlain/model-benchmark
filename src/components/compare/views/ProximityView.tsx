...
export const ProximityView: React.FC<Props> = ({ groups }) => {
	...
	const [layoutMode, setLayoutMode] = useState<
		"cascade" | "row" | "column" | "scatter" | "bloom"
	>("cascade");
	...
	const layoutItems = useMemo(() => {
		...
		const targets = groups.map((_, i) => {
			...
			if (groupCount === 1) {
				...
				ty =
					0.5 +
					Math.sin(angle) *
						(radius * (workspaceSize.width / workspaceSize.height));
			}
			return {
		...
		for (let iter = 0; iter < iterations; iter++) {
			...
			for (let i = 0; i < groupCount; i++) {
				...
				const maxY =
					workspaceSize.height - (g.halfHeight + g.centerOffsetY) - 20;
		...
		<div className="flex flex-col h-full w-full min-h-[800px] bg-zinc-100 dark:bg-zinc-950 rounded-xl overflow-hidden relative">
						...
						{groups.map((g, i) => (
							<div key={i} className="flex items-center gap-2">
								<div
									...
									style={{
										backgroundColor: stringToColor(g.id || g.modelName),
									}}
								/>
								<span
									className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate min-w-0 flex-1"
									title={g.modelName}
								>
									{g.modelName}
								</span>
				...
				{finalItems.length === 0 && (
					<div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-400 p-4 text-center pointer-events-none">
						...
						<div className="text-xs font-mono mt-2 opacity-50">
							Workspace: {workspaceSize.width}x{workspaceSize.height}
							<br />
			...
			<div className="h-10 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center px-4 justify-between text-[10px] text-zinc-400">
				...
				<span className="uppercase tracking-widest">
					Click to enlarge • Hover to isolate
				</span>
			</div>
				...
				<div
					...
					onKeyDown={(e) => {
						...
						if (e.key === "ArrowRight")
							setQuicklookIndex((prev) =>
								prev !== null
									? Math.min(finalItems.length - 1, prev + 1)
									: null,
							);
					}}
					onWheel={(e) => {
				...
				>
					...
					<div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-3 text-white text-sm">
						<span
							...
							style={{
								backgroundColor: `${finalItems[quicklookIndex].color}40`,
								color: finalItems[quicklookIndex].color,
							}}
						>
							{finalItems[quicklookIndex].modelName}
						</span>
						<span className="opacity-60 font-mono">
							#{finalItems[quicklookIndex].seed}
						</span>
						<span className="opacity-40 text-xs">
							{quicklookIndex + 1}/{finalItems.length}
						</span>
					</div>
					...
					{/* Image */}
					<div
						className="max-w-[80vw] max-h-[80vh] relative"
						onClick={(e) => e.stopPropagation()}
					>
						<img
							src={getImageUrl(
								finalItems[quicklookIndex].url,
								finalItems[quicklookIndex].mtime,
							)}
