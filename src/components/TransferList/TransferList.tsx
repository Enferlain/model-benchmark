...
import { FilterMenu, type FilterOptions } from "./FilterMenu";
...
export function TransferList({
		...
		() =>
			libraryModels.filter((m) => {
				...
				if (filters.modelTypes.size > 0) {
					if (!m.model_type || !filters.modelTypes.has(m.model_type))
						return false;
				}
				...
				if (filters.sources.size > 0) {
					...
					const effectiveSource =
						m.source === "Unknown" || !m.source ? "Local" : m.source;
				...
				if (filters.predictionTypes.size > 0) {
					...
					if (!matchesPred && filters.predictionTypes.has("v_prediction")) {
						if (m.prediction_type === "v_prediction" && !m.ztsnr)
							matchesPred = true;
					}
	...
	const handleMoveRight = () => {
		...
		const itemsToMove = libraryModels
	...
	const handleMoveLeft = () => {
		...
		const itemsToRemove = queueModels
			.filter((m) => checkedItems.has(m.id))
			.map((m) => m.id);
	...
	const isDraggingFromLibrary =
		activeModel && !selectedModelIds.includes(activeModel.id);
	const isDraggingFromQueue =
		activeModel && selectedModelIds.includes(activeModel.id);
		...
		>
			<div className="flex flex-col md:flex-row gap-4 h-[600px] w-full items-stretch">
				...
				<ListContainer
					...
					headerAction={
						<PresetMenu currentIds={selectedModelIds} onLoad={onChange} />
					}
				/>
			</div>
